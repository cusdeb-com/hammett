...
This demo showcases the `register_input_handler` decorator provided by Hammett. It allows you to create a handler to process any or a specific type of user input. Check out the documentation for more details.
...
First, navigate to the `demos/unicorn_bot/` directory:

```bash
$ cd demos/unicorn_bot/
```

Next, create and activate a virtual environment:

```bash
$ virtualenv -p python3 unicorn-bot-env
$ source ./unicorn-bot-env/bin/activate
```
...
```bash
$ docker compose build unicorn-bot
$ docker compose run unicorn-bot
```
