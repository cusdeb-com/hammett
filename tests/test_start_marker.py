...
class StartMarkerTests(BaseTestCase):
    ...
    def test_duplicate_keys_last_value_wins(self):
        ...
        assert start_marker['book'] == str(int(_BOOK_ID) + 1)
    ...
    def test_empty_markers_views_are_empty(self):
        ...
        assert len(start_marker.items()) == 0
        assert len(start_marker.keys()) == 0
        assert len(start_marker.values()) == 0
    ...
    def test_items_keys_values_with_multiple_markers(self):
        ...
        assert dict(start_marker.items()) == {
            'source': _SOURCE,
            'book': _BOOK_ID,
            'extra': _EXTRA_VALUE,
        }
        assert set(start_marker.keys()) == {'source', 'book', 'extra'}
        assert set(start_marker.values()) == {_SOURCE, _BOOK_ID, _EXTRA_VALUE}
    ...
    def test_trimming_leading_and_trailing_equals(self):
        ...
        assert start_marker['book'] == _BOOK_ID
        with self.assertRaises(KeyError):
            start_marker['source']
